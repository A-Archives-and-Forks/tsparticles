...
2. Load a base package (for example `@tsparticles/basic`) and call `loadDualityBlueYellowPalette` before `tsParticles.load(...)`
...
(async engine => {
  ...
  await loadDualityBlueYellowPalette(engine);
