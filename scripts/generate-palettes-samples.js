...
const argv = await yargs(cliArgs)
  ...
  .option("dry-run", {
    default: false,
    describe: "Print the matched palettes without taking screenshots.",
    type: "boolean",
  })
  ...
  .option("id", {
    describe: "Capture only specific palette ids/slugs. Accepts repeated flags or comma separated values.",
    type: "array",
  })
  .option("list", {
    default: false,
  ...
  .option("skip-existing", {
    default: false,
    describe: "Skip palettes that already have images/sample.png.",
    type: "boolean",
  })
  .option("timeout", {
    default: 30000,
...
if (matchedItems.length === 0) {
  throw new Error("No palettes matched the provided filters.");
}

if (items.length === 0) {
  console.log("[samples] Nothing to do, every selected palette already has a sample image.");
  process.exit(0);
}
...
try {
  ...
  console.log(`[samples] Captured ${items.length} palette sample image(s).`);
} finally {
  await browser?.close();
  await server.stop();
}
